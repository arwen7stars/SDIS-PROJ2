...
public class InfoChunk {
	...
	public synchronized boolean loadChunksInfo() {
		File file = new File(Peer.PEERS_FOLDER + "/" + Peer.DISK_FOLDER + peer.getID() + "/" + Peer.CHUNKS_INFO);
		if (file.exists()) {
			try {
				ObjectInputStream serverStream = new ObjectInputStream(new FileInputStream(
						Peer.PEERS_FOLDER + "/" + Peer.DISK_FOLDER + peer.getID() + "/" + Peer.CHUNKS_INFO));
	...
	public synchronized void saveChunksInfoFile() {
		try {
			ObjectOutputStream serverStream = new ObjectOutputStream(new FileOutputStream(
					Peer.PEERS_FOLDER + "/" + Peer.DISK_FOLDER + this.peer.getID() + "/" + Peer.CHUNKS_INFO));
			...
			serverStream.writeObject(this.peer.getChunkHosts());
	...
	public void storeChunkInfo(int senderID, String fileID, int chunkNr) {
		...
		CopyOnWriteArrayList<Integer> chunkHosts = this.peer.getChunkHosts().get(hashmapKey);
		...
		if (chunkHosts == null) {
			...
			if (!chunkHosts.contains(senderID)) {
				chunkHosts.add(senderID);
				this.peer.getChunkHosts().replace(hashmapKey, chunkHosts);
